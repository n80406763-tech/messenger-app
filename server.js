...
const VAPID_PUBLIC_KEY =
  ...
  "BNmvLaETpuVL-5ZT46qFp3yy9nfMHoPdaHdjhs78493VV6q6tQnxg4Njl5F_zUot3gOz_bb_p0s87n_4taGUrWI";
const VAPID_PRIVATE_KEY =
  process.env.VAPID_PRIVATE_KEY ||
  "sZ5jXVgmbWlg1QwxSSjwN6qn8Ooy-uZkIwq17CauPc8";
...
function loadState() {
  ...
  try {
    ...
    return {
      ...
      pushInbox:
        parsed.pushInbox && typeof parsed.pushInbox === "object"
          ? parsed.pushInbox
          : {},
      recentClientMessages:
        parsed.recentClientMessages &&
        typeof parsed.recentClientMessages === "object"
          ? parsed.recentClientMessages
          : {},
...
function normalizeLegacyUsers() {
  state.users = state.users.map((u) => ({
    ...
    pushSubscriptions: Array.isArray(u.pushSubscriptions)
      ? u.pushSubscriptions
      : [],
  }));
}
...
function hasSubscriptionEndpoint(endpoint) {
  return state.users.some(
    (u) =>
      Array.isArray(u.pushSubscriptions) &&
      u.pushSubscriptions.some((sub) => sub.endpoint === endpoint)
  );
}
...
function buildPushPayload(message, conversation, recipientId) {
  ...
  const body =
    serialized.text ||
    (serialized.attachment ? "📎 Медиа-сообщение" : "Новое сообщение");
...
async function sendPushToUser(user, payload) {
  ...
  if (payload) {
    user.pushSubscriptions.forEach((sub) =>
      queuePushPayload(sub.endpoint, payload)
    );
    saveState();
  }

  const results = await Promise.all(
    user.pushSubscriptions.map((sub) => sendWebPush(sub))
  );
  ...
  if (alive.length !== user.pushSubscriptions.length) {
    const removed = user.pushSubscriptions.filter(
      (sub) => !alive.some((a) => a.endpoint === sub.endpoint)
    );
...
async function notifyPushParticipants(conversation, senderId, message) {
  const recipients = state.users.filter(
    (u) => u.id !== senderId && canReceiveConversation(conversation, u)
  );
  await Promise.all(
    recipients.map((u) =>
      sendPushToUser(u, buildPushPayload(message, conversation, u.id))
    )
  );
}
...
async function handleApi(req, res, pathname, searchParams = null) {
  ...
  if (req.method === "POST" && pathname === "/api/push/subscribe") {
    ...
    try {
      ...
      const list = Array.isArray(auth.user.pushSubscriptions)
  ...
  if (req.method === "POST" && pathname === "/api/push/unsubscribe") {
    ...
    try {
      ...
      auth.user.pushSubscriptions = (auth.user.pushSubscriptions || []).filter(
  ...
  if (req.method === "GET" && pathname === "/api/push/pull") {
    ...
    if (!hasSubscriptionEndpoint(endpoint))
    ...
    try {
      ...
      if (clientMessageId) {
        ...
        if (existingId) {
          ...
          if (existingMessage) {
            return sendJson(res, 200, {
              message: serializeMessageForViewer(existingMessage, auth.user.id),
              deduped: true,
            });
          }
          delete state.recentClientMessages[key];
      ...
      if (clientMessageId) {
        state.recentClientMessages[
          dedupeKey(auth.user.id, conversation.id, clientMessageId)
        ] = {
