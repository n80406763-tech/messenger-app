...
async function handleApi(req, res, pathname, searchParams = null) {
  ...
  if (req.method === 'POST' && pathname === '/api/conversations/group') {
    ...
    try {
      ...
      const participants = [...new Set([auth.user.id, ...memberIds])].filter((id) =>
        state.users.some((u) => u.id === id)
      );

      if (title.length < 2 || participants.length < 3) {
        return sendJson(res, 400, { error: 'Group title too short or too few participants' });
      }
  ...
  if (req.method === 'POST' && pathname.startsWith('/api/conversations/') && pathname.endsWith('/members/add')) {
    ...
    try {
      ...
      const validIds = [...new Set(memberIds.map((id) => Number(id)).filter((id) => Number.isFinite(id)))].filter(
        (id) => state.users.some((u) => u.id === id)
      );

      conversation.participantIds = [...new Set([...conversation.participantIds, ...validIds])];
      saveState();
      broadcastConversationUpdated(conversation.id);
      return sendJson(res, 200, { conversation: serializeConversation(conversation, auth.user.id) });
    } catch (error) {
