...
function openMediaViewer(attachment) {
  if ((!attachment?.dataUrl && !attachment?.url) || !attachment?.type) return;
  mediaViewerBody.innerHTML = '';
  if (attachment.type === 'image') {
    const img = document.createElement('img');
    img.src = attachment.url || attachment.dataUrl;
    mediaViewerBody.appendChild(img);
  } else if (attachment.type === 'video') {
    const video = document.createElement('video');
    video.src = attachment.url || attachment.dataUrl;
    video.controls = true;
    video.autoplay = true;
...
function messageNode(message) {
  ...
  if (message.attachment) {
    if (message.attachment.type === 'image') {
      const img = document.createElement('img');
      img.src = message.attachment.url || message.attachment.dataUrl;
      img.addEventListener('click', () => openMediaViewer(message.attachment));
      box.appendChild(img);
    }
    if (message.attachment.type === 'video') {
      const video = document.createElement('video');
      video.src = message.attachment.url || message.attachment.dataUrl;
      video.controls = true;
...
function setDialogMeta(conv) {
  ...
  startCallBtn.classList.toggle('hidden', conv.type === 'support');
...
async function openConversation(conversationId) {
  ...
  if (!conv) return;
  if (currentCall && currentCall.conversationId !== conv.id) closeCallMedia();
...
async function loadWebRtcConfig() {
  ...
  webrtcConfig = { iceServers: data.iceServers || [] };
  return webrtcConfig;
}

function resetCallUi() {
  callStatus.textContent = 'Нет активного звонка';
...
async function buildPeerConnection() {
  ...
  pc.onicecandidate = (event) => {
    ...
    sendCallSignal('ice', { candidate: event.candidate }).catch(() => {});
  };
  pc.ontrack = (event) => {
...
async function startOutgoingCall() {
  ...
  showCallModal();
  resetCallUi();
  callStatus.textContent = 'Запрашиваем доступ к камере/микрофону…';
  ...
  try {
    ...
    currentCall = { pc, localStream, conversationId: activeConversation.id };
    await sendCallSignal('offer', { sdp: offer });
    callStatus.textContent = 'Звоним…';
  } catch (error) {
    closeCallMedia();
    callStatus.textContent = `Ошибка звонка: ${error.message}`;
    showCallModal();
  }
}

async function acceptIncomingCall() {
  if (!pendingIncomingSignal || !activeConversation) return;

  showCallModal();
  resetCallUi();
  hangupCallBtn.classList.remove('hidden');
  callStatus.textContent = 'Подключаемся…';
  ...
  try {
    ...
    await pc.setRemoteDescription(new RTCSessionDescription(pendingIncomingSignal.sdp));
    ...
    pendingIncomingSignal = null;
    callStatus.textContent = 'Вызов принят, ждём видео…';
  } catch (error) {
    closeCallMedia();
    callStatus.textContent = `Ошибка принятия: ${error.message}`;
    showCallModal();
  }
}
...
async function handleCallSignal(payload) {
  if (!payload || !activeConversation || payload.conversationId !== activeConversation.id) return;

  if (payload.type === 'offer') {
    pendingIncomingSignal = payload;
    showCallModal();
    callStatus.textContent = `Входящий звонок от ${payload.from || 'собеседника'}`;
  ...
  if (payload.type === 'reject') {
    callStatus.textContent = 'Собеседник отклонил звонок';
    return;
  }
  ...
  if (payload.type === 'answer' && payload.sdp) {
    await currentCall.pc.setRemoteDescription(new RTCSessionDescription(payload.sdp));
    callStatus.textContent = 'Соединение устанавливается…';
    return;
  }

  if (payload.type === 'ice' && payload.candidate) {
    await currentCall.pc.addIceCandidate(new RTCIceCandidate(payload.candidate));
  }
}
    ...
    while (true) {
      ...
      for (const chunk of chunks) {
        ...
        if (event === 'call_signal') {
          handleCallSignal(payload).catch((e) => setChatMessage(e.message, true));
        }
      }
    }
  } catch {
...
startCallBtn?.addEventListener('click', () => {
  if (!window.RTCPeerConnection || !navigator.mediaDevices?.getUserMedia) {
    setChatMessage('WebRTC не поддерживается этим браузером', true);
    return;
  }
  startOutgoingCall().catch((e) => setChatMessage(e.message, true));
});

acceptCallBtn?.addEventListener('click', () => acceptIncomingCall().catch((e) => setChatMessage(e.message, true)));
rejectCallBtn?.addEventListener('click', () => rejectIncomingCall().catch((e) => setChatMessage(e.message, true)));
hangupCallBtn?.addEventListener('click', () => {
  sendCallSignal('bye').catch(() => {});
  closeCallMedia();
  callModal.classList.add('hidden');
});
closeCallBtn?.addEventListener('click', () => callModal.classList.add('hidden'));
